        ...
        <v-col cols="12" xs="12" sm="6" md="4" v-for="(product, i) in products" :key="i">
            <v-card class="mx-auto rounded-xl" max-width="300" flat outlined>
                <div justify="center" align="center">
                    <v-img :src="product.img" max-height="300" max-width="300" contain></v-img>
                </div>
                <v-card-title>
                    {{product.title}}
                </v-card-title>
                <v-card-title class="grey--text text-grey-darken-1 caption mt-n6">{{product.subtitle}}
                </v-card-title>
                <v-card-title class="mt-n4">{{product.price}}</v-card-title>
                <v-card-actions class="mw-2 mt-n4 ml-2">
                    <v-btn outlined class="mt-n2 add">
                        <v-icon color="green" @click="decrement">mdi-minus</v-icon>
                    </v-btn>
                    <strong class="mx-2 mt-n2" v-text="bpm"></strong>
                    <v-btn outlined class="mt-n2 add">
